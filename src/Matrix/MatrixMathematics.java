...
public class MatrixMathematics {
	...
	public static double determinant(Matrix matrix) throws NoSquareException {
		...
		for (int i=0; i<matrix.getNcols(); i++) {
			sum += sign(i) * matrix.getValueAt(0, i) * determinant(createSubMatrix(matrix, 0, i));
		}
		return sum;
	}
	...
	public static Matrix cofactor(Matrix matrix) throws NoSquareException {
		...
		for (int i=0;i<matrix.getNrows();i++) {
			for (int j=0; j<matrix.getNcols();j++) {
				mat.setValueAt(i, j, sign(i) * sign(j) * determinant(createSubMatrix(matrix, i, j)));
			}
		}
	...
	public static Matrix normalize1x4(Matrix matrix){
		assert matrix.getValues().length == 1;
		assert matrix.getValues()[0].length == 4;
		
		double data[][] = matrix.getValues();
		for(int i=0;i<4;i++){
			data[0][i] = data[0][i]/data[0][3];
		}
